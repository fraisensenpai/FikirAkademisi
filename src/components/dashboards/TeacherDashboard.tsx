...
import { Users, BookOpen, TrendingUp, BarChart3, Clock, Sparkles, User, Monitor, Library, CheckCircle2 } from "lucide-react";
    ...
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 glass-premium p-8 border-primary/10 shadow-2xl shadow-primary/5">
        <div className="space-y-2">
          ...
          <h2 className="text-3xl font-display font-black text-foreground uppercase tracking-tight">Hoca Paneli</h2>
          <p className="text-muted-foreground font-medium italic">Öğrencilerinizin okuma verilerini ve gelişimlerini anlık takip edin.</p>
        </div>
      ...
      <div className="glass-premium border-white/5 shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/5">
          <h3 className="font-bold text-foreground flex items-center gap-3 uppercase tracking-widest text-sm">
            ...
            Öğrenci İlerlemeleri & Kaynak Ayrımı
          </h3>
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-secondary rounded-full shadow-[0_0_8px_rgba(var(--secondary),0.5)]" />
                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-tighter">Sistem (PDF)</span>
            </div>
            <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-blue-400 rounded-full shadow-[0_0_8px_rgba(96,165,250,0.5)]" />
                <span className="text-[10px] font-black text-muted-foreground uppercase tracking-tighter">Manuel (Aktarım)</span>
            </div>
          </div>
        </div>
        
        {studentProgress.length === 0 ? (
          <div className="p-20 text-center text-muted-foreground italic font-medium">Baksak da göremeyiz, henüz veri akışı yok...</div>
        ) : (
          <ScrollArea className="h-[500px]">
            <div className="divide-y divide-white/5">
              {studentProgress.map((sp, i) => {
                ...
                // Site pages = current_page - manual_pages (capped at 0)
                  ...
                  <div key={i} className="p-6 flex flex-col md:flex-row md:items-center gap-6 hover:bg-white/5 transition-all group">
                    <div className="flex items-center gap-4 w-64 flex-shrink-0">
                        ...
                        <div className="min-w-0">
                            <p className="font-bold text-foreground truncate uppercase text-sm leading-none mb-1 group-hover:text-secondary transition-colors">{sp.user?.full_name || "—"}</p>
                            <p className="text-[10px] font-black text-muted-foreground/50 uppercase tracking-tighter">{sp.book?.title}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 space-y-2">
                      <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 leading-none">
                        <div className="flex gap-3">
                            <span className="flex items-center gap-1.5"><Monitor className="w-3 h-3 text-secondary" /> Biten: %{Math.round(sitePercent)}</span>
                            <span className="flex items-center gap-1.5"><Library className="w-3 h-3 text-blue-400" /> Aktarım: %{Math.round(manualPercent)}</span>
                        </div>
                        <span className="text-foreground font-black italic">TOPLAM: %{totalPercent}</span>
                      </div>
                      
                      <div className="relative h-3 w-full bg-background rounded-full overflow-hidden border border-white/5 group-hover:border-white/10 transition-colors">
                        {/* Site Progress (Primary/Secondary) */}
                        <div 
                          className="absolute h-full bg-secondary shadow-[0_0_10px_rgba(var(--secondary),0.3)] transition-all duration-1000 ease-out z-20"
                          style={{ width: `${sitePercent}%` }}
                        />
                        {/* Manual Progress (Blue) */}
                        <div 
                          className="absolute h-full bg-blue-400 opacity-80 transition-all duration-1000 ease-out z-10"
                    ...
                    <div className="flex items-center gap-6 w-32 justify-end">
                        <div className="text-right">
                             <p className="text-lg font-black text-foreground leading-none mb-1 group-hover:text-secondary transition-all">
                                {Math.round(Number(sp.total_minutes))}
                             </p>
                             <p className="text-[10px] font-black text-muted-foreground/40 uppercase tracking-tighter">Dakika</p>
                        </div>
                        <div className={`p-2 rounded-xl border ${sp.is_completed ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-muted border-white/5 text-muted-foreground opacity-30'} transition-all`}>
                            {sp.is_completed ? <CheckCircle2 className="w-5 h-5" /> : <Clock className="w-5 h-5" />}
                        </div>
                    </div>
                  </div>
                );
