...
import { Send, BookOpen, User, Users, Search, Trash2, ArrowLeft, Check, CheckCheck, Smile, Info } from "lucide-react";
...
export default function Messages() {
  ...
  const fetchItems = async () => {
    ...
    // 2. Üyesi olduğun ve Sahibi olduğun grupları çek
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    if (error) {
      console.error(error);
      toast.error("SQL hatası: Lütfen receiver_id DROP NOT NULL sorgusunu çalıştırın.");
    } else {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] bg-background/50 backdrop-blur-xl rounded-3xl border border-white/5 overflow-hidden shadow-2xl relative">
      ...
      <div className={`flex-1 flex flex-col relative overflow-hidden bg-gradient-to-b from-transparent to-primary/5 ${!selectedRecipient ? "hidden md:flex" : "flex animate-in slide-in-from-right duration-300"}`}>
        {selectedRecipient ? (<><div className="p-4 md:p-6 border-b border-white/5 flex items-center justify-between bg-background/80 backdrop-blur-md z-10">
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
              <Avatar className={`h-10 w-10 ring-2 ${selectedRecipient.isGroup ? "ring-indigo-500/30 rounded-xl bg-indigo-500/10" : "ring-primary/20"}`}><AvatarFallback className={selectedRecipient.isGroup ? "text-indigo-400" : ""}>{selectedRecipient.isGroup ? <Users className="w-6 h-6" /> : <User className="w-6 h-6" />}</AvatarFallback></Avatar>
            ...
            <div>
              <h3 className="font-bold text-foreground leading-tight uppercase">{selectedRecipient.full_name}</h3>
              <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-tighter">{selectedRecipient.isGroup ? "Grup Sohbeti" : (isUserActive(selectedRecipient.last_seen_at) ? "Aktif" : selectedRecipient.role)}</p>
            </div>
          </div>
          {selectedRecipient.isGroup && (
            <Popover>
              <PopoverTrigger asChild><Button variant="ghost" size="icon" className="hover:text-indigo-400"><Info className="w-5 h-5" /></Button></PopoverTrigger>
              <PopoverContent className="w-64 glass-premium p-4 rounded-2xl border-white/10 shadow-2xl"><h4 className="text-xs font-black uppercase tracking-widest text-indigo-400 mb-3">Grup Üyeleri ({groupMembers.length})</h4><div className="space-y-2 max-h-60 overflow-y-auto">{groupMembers.map(m => (<div key={m.id} className="flex items-center gap-2"><div className={`w-1.5 h-1.5 rounded-full ${m.role === 'teacher' ? 'bg-secondary' : 'bg-primary'}`} /><span className="text-sm font-medium">{m.full_name}</span></div>))}</div></PopoverContent>
            </Popover>
          )}</div>
            <ScrollArea className="flex-1 p-6"><div className="space-y-6">
              {messages.map(msg => (<div key={msg.id} className={`flex flex-col group ${msg.sender_id === user?.id ? "items-end" : "items-start"}`}>
                {selectedRecipient.isGroup && msg.sender_id !== user?.id && (<span className="text-[10px] font-black text-indigo-400/70 mb-1 ml-4 uppercase tracking-tighter">{msg.sender?.full_name}</span>)}
                <div className="max-w-[80%] space-y-1 relative">
                  <div className="flex items-center gap-2">
                    ...
                    <div className={`p-4 rounded-2xl shadow-xl transition-all ${msg.sender_id === user?.id ? "bg-primary text-primary-foreground rounded-tr-none" : "bg-muted/80 backdrop-blur-md rounded-tl-none border border-white/5"}`}><p className="text-sm whitespace-pre-wrap">{msg.content}</p></div>
                    {msg.sender_id !== user?.id && (<Popover><PopoverTrigger asChild><Button variant="ghost" size="icon" className="h-8 w-8 hover:text-primary"><Smile className="w-4 h-4" /></Button></PopoverTrigger><PopoverContent className="w-auto p-1 flex gap-1 rounded-full border-white/5 bg-background/90 backdrop-blur-md shadow-2xl">{COMMON_EMOJIS.map(emoji => (<button key={emoji} onClick={() => toggleReaction(msg.id, emoji)} className="hover:scale-125 transition-transform p-1.5 text-lg">{emoji}</button>))}</PopoverContent></Popover>)}
                  </div>
                  {renderReactions(msg)}
                  <div className="flex items-center justify-end gap-1 px-1"><span className="text-[9px] opacity-30 font-mono">{new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>{!selectedRecipient.isGroup && msg.sender_id === user?.id && (<span className={`text-[8px] font-black uppercase ${msg.is_read ? "text-blue-500" : "opacity-30"}`}>{msg.is_read ? "Görüldü" : "İletildi"}</span>)}</div>
                </div></div>))}
              <div ref={scrollRef} /></div></ScrollArea>
            <div className="p-6 bg-background/80 backdrop-blur-xl border-t border-white/5"><form onSubmit={sendMessage} className="flex gap-4"><Input value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder={`${selectedRecipient.full_name} sohbetine yaz...`} className="flex-1 bg-white/5 border-white/10 h-14 px-6 rounded-2xl" /><Button type="submit" size="lg" className="h-14 w-14 rounded-2xl"><Send className="w-6 h-6" /></Button></form></div></>) : (<div className="flex-1 flex flex-col items-center justify-center text-muted-foreground opacity-20 p-20 text-center"><Users className="w-32 h-32 mb-8" /><h2 className="text-3xl font-display font-bold">Bir Sohbet Seç</h2><p className="max-w-xs mt-4">Arkadaşlarınla veya grubunla hemen yazışmaya başla.</p></div>)}
      </div>
    </div>
  );
}
